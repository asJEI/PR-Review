/**
 * Export risk review report as UTF-8 JSON.
 *
 * Usage:
 *   node packages/ai/scripts/export-risk-review.mjs <pr-url> [output.json]
 */
...
import { generateRiskReview } from "../dist/index.js";
...
if (!prUrl) {
  console.error("Usage: node packages/ai/scripts/export-risk-review.mjs <pr-url> [output.json]");
  process.exit(1);
}
...
const outputPath = resolve(process.cwd(), outputArg ?? "risk-review.json");
...
const riskReview = await generateRiskReview({
...
writeFileSync(outputPath, `\uFEFF${JSON.stringify(riskReview, null, 2)}`, "utf8");
console.error(`Wrote UTF-8 risk review to ${outputPath}`);
